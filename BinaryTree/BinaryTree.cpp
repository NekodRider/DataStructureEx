...
status ElemDisplay(ElemType &e){
	printf("%d",e);
	return OK;
}
status ElemPlus(ElemType &e){
	e++;
	return OK;
}
...
status CreateBiTree(BinaryTree &T)
{
	...
	{
		if (!(T = (BinaryTree)malloc(sizeof(Node))))
			exit(OVERFLOW);
		(*T).node_name = define;
		(*T).data = 0;
...
BinaryTree RightSibling(BinaryTree T, char e)
{
	if (!T)
	{
		printf("请先初始化二叉树！");
